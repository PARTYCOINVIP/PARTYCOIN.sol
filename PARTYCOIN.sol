...
contract Ownable is Context {
    address public _owner = {REDACTED};
...
contract PARTYCOIN is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 2000000 * 10**6 * 10**6;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
     
    string private _name = "PARTYCOIN";
    string private _symbol = "{REDACTED}";
    ...
    uint256 public _maxTxAmount = 50000 * 10**5 * 10**6;
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 liquidity, uint256 Burn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _Burn(uint256 burn) private {
        uint256 currentRate =  _getRate();
        uint256 rBurn = burn.mul(currentRate);
        _rOwned[address(0x000000000000000000000000000000000000dEaD)] = _rOwned[address(0x000000000000000000000000000000000000dEaD)].add(rBurn);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(takeFee){
        emit Transfer(sender, address(0x000000000000000000000000000000000000dEaD), Burn);}
